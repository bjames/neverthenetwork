...
2. Modify in Place
	...
	2. NXOS counts `remark` entries as ACL lines, IOS does not. When you modify in place, you need to be mindful of this. If you use remarks, line numbers on IOS and NXOS won't match, so you'll need to create distinct script configuration files for each device type. 
...
`mod_acl` implements both of these methods.
...
4. Use Pool.map() to spawn _n_ processes (where in _n_ is the number of threads specified in the YAML file).[^2]
...
[^2]: If _m_ > _n_, where _m_ is the number of devices, the device list is broken into chunks of _m_/_n_ and is iterated by Pool.map()