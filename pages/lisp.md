...
Map-Requests are primarily used to request EID-to-RLOC mappings and Map-Replies are used to provide mappings. [RFC 6833](https://tools.ietf.org/html/rfc6833) defines Map-Resolvers and Map-Servers as two separate services that proxy Map-Requests and Map-Replies. Map-Resolvers proxy Map-Requests sent from ITRs and Map-Servers proxy Map-Replies sent from ETRs. When a Map-Resolver doesn't have an entry for an EID and can't determine that the EID is non-existant, it forwards the request to an authoritative Map-Server, which responds to the request directly[^2]. 

When a Map-Resolver is in use the ITR does not need to be connected to the mapping database. Likewise, when a Map-Server is in use the ETR no longer needs to be connected to the mapping service. A single device can operate as both a Map-Resolver and Map-Server.While ITRs and ETRs can connect to the Mapping-Service and send Map-Requests and Map-Replies directly, this could cause an overwhelming increase in load. In practice, we are likely to proxy these messages using Map-Resolvers and Map-Servers.
...
The Internet is the obvious and intended use case for LISP. LISP provides a potential solution to the exponential increase in routes on the internet. As IPv6 adoption increases, so will the size of intent routing tables. In addition, LISP could potentially extend the life of IPv4 by allowing the use of prefixes longer than /24 without carrier lock-in. 
...
All IP networks are similar, so something useful for the Internet may also be useful for the enterprise. On the surface, this seems less true for LISP than it is for BGP. LISP provides a solution for the exponential growth of routes. Cisco ASR RP3 route processors can handle up to 8.5 million IPv4 routes[^3]. It's unlikely that many enterprises are close to that and those that are will have the budget to run the latest and greatest hardware. Using LISP to avoid advertising routes into an enterprise core is generally going to just add needless complexity.
...
LISP is already being used by enterprises that have implemented Cisco SDA[^4]. SDA uses an _enhanced_ version of LISP for Control-Plane traffic, in roughly the way I described above. SDA routers only maintain local routes and use LISP to locate endpoints on the SDA fabric. Cisco uses VXLAN instead of LISP to encapsulate traffic from endpoints so that Layer 2 headers stay intact. It's possible that other Software Defined Networking solutions use LISP in some way, but I'm not aware of any.

# Conclusions

Whether or not it will actually be adopted on the Internet remains to be seen. 

## Glossery of LISP Terms
